...
function generateAALink(segments: Segment[], pax: number): string {
  ...
  // Passenger mix block: A# = adults count (1-9)
  const n = Math.min(9, Math.max(1, Math.floor(pax) || 1));
  const paxCode = `A${n}S0C0I0Y0L0`;

  const header = `GOOGLE,0,US,multi,${numSeg},${paxCode},0,${firstSeg.orig},0,${lastOfFirstDir.dest},0,0,0,0,0,0,0,1.00,${n},`;
...
export default function Home() {
  ...
  // Always use current passenger count; open reliably
  const handleOpenLink = () => {
    const url = generateAALink(segments, parseInt(pax) || 1) || generatedUrl;
    if (!url) return;
    setGeneratedUrl(url);
